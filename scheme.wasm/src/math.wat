...
(func $sub (param $env i32) (param $args i32) (result i32)
  (local $accum i64)
  (local $car i32)
  ...
  (if (i32.ne (%get-type $args) (%cons-type))
    (then (return (%alloc-error (%sym-32 0x73677261 4) (local.get $args))))
  )

  (local.set $car (%car-l $args))
  (if (i32.ne (%get-type $car) (%i64-type))
    (then (return (%alloc-error (%sym-32 0x73677261 4) (local.get $car))))
  )
